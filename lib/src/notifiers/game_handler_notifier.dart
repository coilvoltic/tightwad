...
class GameHandlerNotifier extends ChangeNotifier {
  ...
  GameHandlerNotifier()
  {
    ...
    computeRandomMatrix(levels[lvl].getSqNbOfTiles, levels[lvl].getMatrixElementMaxValue, levels[lvl].getSmallMatrixElementMaxValue);    
  ...
  {
    bool broken = false;
    for (int i = 0; i < pow(levels[lvl].getSqNbOfTiles,levels[lvl].getSqNbOfTiles); i++)
    {
      List<int> checkPermutation = List.filled(levels[lvl].getSqNbOfTiles, 0, growable: false);
      broken = false;
      for (int j = 0; j < levels[lvl].getSqNbOfTiles; j++)
      {
        checkPermutation[j] = (i/(pow(levels[lvl].getSqNbOfTiles,j))-1).floor() % levels[lvl].getSqNbOfTiles + 1;
        for (int k = 0; k < j; k++)
        {
  ...
  {
    possiblePermutations.removeWhere((element) => element[move.x-1] == move.y);
  }

  void updateForbiddenMoveListFromUser(Coordinates move)
  {
    userPossibleMoveList.removeWhere((element) => element.x == move.x && element.y == move.y);

    for (int i = 0; i < levels[lvl].getSqNbOfTiles; i++)
    {
      if (move.x != i+1)
      {
        userPossibleMoveList.removeWhere((element) => element.x == i+1 && element.y == move.y);
      }
    }
    ...
    for (int j = 0; j < levels[lvl].getSqNbOfTiles; j++)
    {
      if (move.y != j+1)
      {
        userPossibleMoveList.removeWhere((element) => element.x == move.x && element.y == j+1);
      }
    }
  }
  ...
  {
    if (nbUserPress == levels[lvl].getSqNbOfTiles - 2 && levels[lvl].getFirstPlayer == Player.algo ||
        nbAlgoPress == levels[lvl].getSqNbOfTiles - 2 && levels[lvl].getFirstPlayer == Player.user)
    {
  ...
  {
    return !userPossibleMoveList.any((item) => item.x == move.x && item.y == move.y) &&
  ...
  void computeAlgoNextMove()
  {
    ...
    for (int i = 0; i < possiblePermutations.length; i++)
    {
      ...
      for (int j = 0; j < levels[lvl].getSqNbOfTiles; j++)
      {
        sum = sum + getMatrixElement(convertPermElementToCoordinates(possiblePermutations[i][j], j));
      }
    ...
    for (int j = 0; j < levels[lvl].getSqNbOfTiles; j++)
    {
      Coordinates potentialNextMove = convertPermElementToCoordinates(possiblePermutations[greatestPermutationIndex][j], j);
      if (userPossibleMoveList.any((item) => item.x == potentialNextMove.x && item.y == potentialNextMove.y) &&
          !(algoMovesList.any((item) => item.x == potentialNextMove.x && item.y == potentialNextMove.y)))
      {
    ...
    else
    {
      ...
      algoNextMove = convertPermElementToCoordinates(possiblePermutations[greatestPermutationIndex][greatestElementOfPermutationIndex], greatestElementOfPermutationIndex);
    }
  ...
  {
    possiblePermutations.removeWhere((element) => element[algoNextMove.x-1] != algoNextMove.y);
  }
  ...
  {
    userPossibleMoveList.removeWhere((element) => element.x == algoNextMove.x && element.y == algoNextMove.y);
  }

  void checkEndGame()
  {
    if (nbUserPress == levels[lvl].getSqNbOfTiles && nbAlgoPress == levels[lvl].getSqNbOfTiles)
    {
  ...
  List<int> computeRow(int sqDim, int stSmallIndex, int ndSmallIndex, int matrixElementMaxValue, int smallMatrixElementMaxValue)
  {
  ...
  void computeRandomMatrix(int sqDim, int matrixElementMaxValue, int smallMatrixElementMaxValue)
  {
    ...
    for (int i = 0; i < sqDim; i++)
    {
      ...
      if (i == sqDim-1-1 && baseElements.any((element) => element == sqDim))
      {
    ...
    for (int i = 0; i < sqDim; i++)
    {
      matrix.add(computeRow(sqDim, stSmall[i]-1, ndSmall[i]-1, matrixElementMaxValue, smallMatrixElementMaxValue));
    }
  }
