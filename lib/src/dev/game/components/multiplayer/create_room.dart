...
class _CreateRoomState extends State<CreateRoom> {
  ...
  void computeNewPaddingForCustomThumb() {
    ...
    _customThumbPadding = 15.0 + _sliderValue * (MediaQuery.of(context).size.width * Utils.ROOM_LOOBY_WIDTH_LIMIT_RATIO - 31.0 - 15.0);
  }

  Widget buildRoundsGauge() {
    return AnimatedContainer(
      duration: const Duration(milliseconds: Utils.THEME_ANIMATION_DURATION_MS),
      width: MediaQuery.of(context).size.width * Utils.ROOM_LOOBY_WIDTH_LIMIT_RATIO,
      height: Utils.TEXT_FIELD_HEIGHT,
  ...
  Widget buildNbOfRoundsStatement() {
    return SizedBox(
      ...
      child: Text('number of rounds: $_nbOfRounds',
        style: GoogleFonts.bebasNeue(
          decoration: TextDecoration.none,
          fontSize: min(min(MediaQuery.of(context).size.width,
                        MediaQuery.of(context).size.height),
