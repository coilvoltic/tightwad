enum GameTheme { dark, light, diamond, none }

enum GameStatus { playing, lose, win, draw, nextlevel, tryagain, finish, loading, error, winsession, losesession, drawsession, retry, leavewon, leavelostordraw, alone, none }

enum VolumeMode { up, off }

enum Player { user, algo, none, creator, guest }

enum Entity { tutorial, singleplayerwelcome, multiplayerwelcome, singleplayergame, multiplayergame, waitingopponent, lobby, none }

enum TutorialPhase { welcome, learning, finished }

enum OptionButtonType { main, theme, mode, volume }

enum MultiPlayerStatus { creator, guest, none }

enum RoundStatus { won, lost, draw, none }
